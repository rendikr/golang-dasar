...
func TestContextWithTimeout(t *testing.T) {
	...
	parent := context.Background()
	ctx, cancel := context.WithTimeout(parent, 5*time.Second) // set the process should not be exceeding the maximum time, will trigger cancel() automatically after n time
	defer cancel()
...
func TestContextWithDeadline(t *testing.T) {
	...
	parent := context.Background()
	ctx, cancel := context.WithDeadline(parent, time.Now().Add(5*time.Second)) // set the process should not be exceeding the maximum time, will trigger cancel() automatically at a specific time
	defer cancel()
